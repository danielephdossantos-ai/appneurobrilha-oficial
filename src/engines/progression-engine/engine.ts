...
export class ProgressionEngine {
  ...
  static async updateMastery(
    ...
    const old_success_count = Math.round(
      ((current?.success_rate || 0) * (current?.total_attempts || 0)) / 100,
    );
    ...
    const { error } = await supabase.from("child_skill_mastery").upsert(
      {
        child_id: childId,
  ...
  static async scheduleNextReview(childId: string, activityId: string, score: number) {
    ...
    const { error } = await supabase.from("spaced_repetition_schedule").upsert(
      {
        child_id: childId,
  ...
  private static async updateGeneralStats(childId: string, materia: string) {
    ...
    const evolution_percentage =
      skills.reduce((acc, s) => acc + Number(s.mastery_level), 0) / skills.length;
    ...
    const { error } = await supabase.from("child_progression_stats").upsert(
      {
        child_id: childId,
        materia,
