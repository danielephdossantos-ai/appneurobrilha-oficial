...
export class ActivityEngine {
  ...
  private static selecionarVariacao(
    template: ActivityTemplate,
    difficulty: DifficultyLevel,
  ): GeneratedActivity["content"] {
    // Lógica para evitar repetição e selecionar itens do banco
    const availableContent = CONTENT_DATABASE.filter(
      (item) =>
        template.contentPool.includes(item.id) ||
        (template.requiredSkills.some((s) => item.tags.includes(s)) &&
          !this.usedContentIds.has(item.id)),
    );
  ...
  private static gerarOpcoes(correct: ContentItem, difficulty: DifficultyLevel): ActivityOption[] {
    const distractors = CONTENT_DATABASE.filter(
    ...
    const options: ActivityOption[] = [
      {
        ...
        type: (correct.type === "image" ? "image" : "text") as "image" | "text",
        isCorrect: true,
      },
      ...distractors.map((d, i) => ({
        id: `dist-${i}`,
