...
export const createChild = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const req = getRequest();
    const userId = getUserIdFromRequest(req);
    const [row] = await db.insert(children).values({ ...(data.child as any), userId }).returning();
    return row;
  });
...
export const getChildAnamnesis = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const [row] = await db.select().from(childAnamnesis).where(eq(childAnamnesis.childId, data.childId));
    return row ?? null;
  });
...
export const saveChildAnamnesis = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const [existing] = await db.select().from(childAnamnesis).where(eq(childAnamnesis.childId, data.child_id));
    if (existing) {
      ...
      const [updated] = await db.update(childAnamnesis)
        .set({ responses: data.responses, internalProfile: data.internal_profile ?? {}, editCount: count + 1, updatedAt: new Date() })
        .where(eq(childAnamnesis.id, existing.id)).returning();
      await db.update(children).set({ anamnesisEditCount: count + 1, updatedAt: new Date() }).where(eq(children.id, data.child_id));
      return updated;
    } else {
      const [inserted] = await db.insert(childAnamnesis)
        .values({ childId: data.child_id, responses: data.responses, internalProfile: data.internal_profile ?? {}, editCount: 1 })
        .returning();
      await db.update(children).set({ anamnesisId: inserted.id, anamnesisEditCount: 1, anamneseCompleta: true, updatedAt: new Date() }).where(eq(children.id, data.child_id));
      return inserted;
    }
  });
