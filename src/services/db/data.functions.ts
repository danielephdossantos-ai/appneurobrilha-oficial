...
import {
  ...
  mascots, userMascots, gamificationProfiles, mascotStates,
...
function getUid(req: Request) { return req.headers.get("x-replit-user-id") ?? "anonymous"; }
...
export const createNotification = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const userId = getUid(getRequest());
    const [row] = await db.insert(notifications).values({ ...(data.notif as any), userId }).returning();
    return row;
  });
...
export const markNotificationRead = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const userId = getUid(getRequest());
    await db.update(notifications).set({ read: true }).where(and(eq(notifications.id, data.id), eq(notifications.userId, userId)));
    return { ok: true };
  });
...
export const getStudyAgenda = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    return db.select().from(studyAgenda).where(eq(studyAgenda.childId, data.childId)).orderBy(asc(studyAgenda.createdAt));
  });
...
export const upsertStudyAgendaItem = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    if (data.id) {
      const [row] = await db.update(studyAgenda).set({ ...(data.item as any), updatedAt: new Date() }).where(eq(studyAgenda.id, data.id)).returning();
      return row;
    }
    const [row] = await db.insert(studyAgenda).values(data.item as any).returning();
    return row;
  });
...
export const createExamMission = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const { exam_study_plans: plans, exam_mission_contents: contents, ...mission } = data.mission as any;
...
export const updateExamMissionContent = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const [row] = await db.update(examMissionContents).set(data.patch as any).where(eq(examMissionContents.id, data.id)).returning();
    return row;
  });
...
export const updateExamStudyPlan = createServerFn({ method: "POST" })
  .inputValidator(z.object({ id: z.string(), patch: z.record(z.unknown()) }))
  .handler(async ({ data }) => {
    const [row] = await db.update(examStudyPlans).set(data.patch as any).where(eq(examStudyPlans.id, data.id)).returning();
    return row;
  });
...
export const upsertUserMascot = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const userId = getUid(getRequest());
    if (data.id) {
      const [row] = await db.update(userMascots).set(data.mascot as any).where(and(eq(userMascots.id, data.id), eq(userMascots.userId, userId))).returning();
      return row;
    }
    const [row] = await db.insert(userMascots).values({ ...(data.mascot as any), userId }).returning();
    return row;
  });
...
export const getGamificationProfile = createServerFn({ method: "POST" })
  .inputValidator(z.object({ childId: z.string() }))
  .handler(async ({ data }) => {
    const [row] = await db.select().from(gamificationProfiles).where(eq(gamificationProfiles.childId, data.childId));
    return row ?? null;
  });
...
export const upsertGamificationProfile = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const [row] = await db.insert(gamificationProfiles).values(data.profile as any)
      .onConflictDoUpdate({ target: gamificationProfiles.childId, set: { ...(data.profile as any), updatedAt: new Date() } }).returning();
    return row;
  });
...
export const getMascotState = createServerFn({ method: "POST" })
  .inputValidator(z.object({ childId: z.string() }))
  .handler(async ({ data }) => {
    const [row] = await db.select().from(mascotStates).where(eq(mascotStates.childId, data.childId));
    return row ?? null;
  });
...
export const upsertMascotState = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const [row] = await db.insert(mascotStates).values(data.state as any)
      .onConflictDoUpdate({ target: mascotStates.childId, set: { ...(data.state as any), updatedAt: new Date() } }).returning();
    return row;
  });
...
export const getActivityLogs = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    return db.select().from(activityLogs).where(eq(activityLogs.childId, data.childId))
      .orderBy(desc(activityLogs.createdAt)).limit(data.limit ?? 50);
  });

export const insertActivityLog = createServerFn({ method: "POST" })
  .inputValidator(z.object({ log: z.record(z.unknown()) }))
  .handler(async ({ data }) => {
    const [row] = await db.insert(activityLogs).values(data.log as any).returning();
    return row;
  });
...
export const upsertChildSkillMastery = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const [row] = await db.insert(childSkillMastery).values(data.mastery as any)
      .onConflictDoUpdate({ target: [childSkillMastery.childId, childSkillMastery.skillCode], set: { ...(data.mastery as any), updatedAt: new Date() } }).returning();
    return row;
  });
...
export const getChildProgressionStats = createServerFn({ method: "POST" })
  .inputValidator(z.object({ childId: z.string() }))
  .handler(async ({ data }) => {
    return db.select().from(childProgressionStats).where(eq(childProgressionStats.childId, data.childId));
  });

export const upsertChildProgressionStats = createServerFn({ method: "POST" })
  .inputValidator(z.object({ stats: z.record(z.unknown()) }))
  .handler(async ({ data }) => {
    const [row] = await db.insert(childProgressionStats).values(data.stats as any)
      .onConflictDoUpdate({ target: [childProgressionStats.childId, childProgressionStats.materia], set: { ...(data.stats as any), updatedAt: new Date() } }).returning();
    return row;
  });
...
export const getCognitiveProfile = createServerFn({ method: "POST" })
  .inputValidator(z.object({ childId: z.string() }))
  .handler(async ({ data }) => {
    const [row] = await db.select().from(cognitiveProfile).where(eq(cognitiveProfile.childId, data.childId));
    return row ?? null;
  });
...
export const upsertCognitiveProfile = createServerFn({ method: "POST" })
  .inputValidator(z.object({ profile: z.record(z.unknown()) }))
  .handler(async ({ data }) => {
    const [row] = await db.insert(cognitiveProfile).values(data.profile as any)
      .onConflictDoUpdate({ target: cognitiveProfile.childId, set: { ...(data.profile as any), updatedAt: new Date() } }).returning();
    return row;
  });
...
export const insertLongitudinalScore = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const [row] = await db.insert(longitudinalScores).values(data.score as any).returning();
    return row;
  });
...
export const upsertUserPrivacySettings = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const userId = getUid(getRequest());
    const [row] = await db.insert(userPrivacySettings).values({ ...(data.settings as any), userId })
      .onConflictDoUpdate({ target: userPrivacySettings.userId, set: { ...(data.settings as any), updatedAt: new Date() } }).returning();
    return row;
  });
...
export const upsertAnamneseV2 = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const userId = getUid(getRequest());
    const [row] = await db.insert(anamneseV2).values({ ...(data.row as any), userId })
      .onConflictDoUpdate({ target: anamneseV2.childId, set: { ...(data.row as any), updatedAt: new Date() } }).returning();
    return row;
  });
...
export const getBnccHabilidades = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    return rows.filter(r => (!data.ano || r.ano === data.ano) && (!data.disciplina || r.disciplina === data.disciplina));
  });
...
export const getPedagogicalActivities = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    return rows.filter(r => (!data.serie || r.serie === data.serie) && (!data.materia || r.materia === data.materia));
  });
...
export const upsertProgressoAluno = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const [row] = await db.insert(progressoAluno).values(data.progresso as any)
      .onConflictDoUpdate({ target: [progressoAluno.alunoId, progressoAluno.codigoBncc], set: { ...(data.progresso as any), updatedAt: new Date() } }).returning();
    return row;
  });
