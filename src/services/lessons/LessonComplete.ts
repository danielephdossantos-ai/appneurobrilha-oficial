...
export class LessonComplete {
  ...
  static async load(codigoBNCC: string): Promise<LessonCompleteModel> {
    ...
    const [
      ...
      LessonExplanationService.list(codigoBNCC).catch(() => []),
      ...
      LessonAssessmentService.list(codigoBNCC).catch(() => []),
      ...
      LearningObjectivesService.listByCodigoBncc(codigoBNCC).catch(() => []),
