...
async function callLovable(opts) {
    ...
    if (!key)
        return { ok: false, motivo: "erro", detalhe: "LOVABLE_API_KEY ausente" };
    try {
        const body = {
        ...
        if (!text)
            return { ok: false, motivo: "erro", detalhe: "resposta vazia" };
        return { ok: true, text };
    }
    catch (e) {
...
async function callGroq(opts) {
    ...
    if (!key)
        return { ok: false, motivo: "erro", detalhe: "GROQ_API_KEY ausente" };
    try {
        const body = {
 ...
 */
export async function chatCompletionFallback(opts) {
    const primaria = await callGroq(opts);
    if (primaria.ok)
        return { ok: true, text: primaria.text, fonte: "groq" };
    console.warn(`[ai-fallback]${opts.label ? " " + opts.label : ""} Groq falhou (${primaria.motivo}) — tentando Lovable`);
    const secundaria = await callLovable(opts);
    if (secundaria.ok)
        return { ok: true, text: secundaria.text, fonte: "lovable" };
