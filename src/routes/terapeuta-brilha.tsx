...
function Terapeuta() {
  ...
  const [msgs, setMsgs] = useState<{ role: "ai" | "user"; t: string }[]>([
    {
      role: "ai",
      t: `Oi! Sou ${mascotName}. Hoje vou te ajudar como terapeuta com ${activeChild?.nome ?? "sua criança"}. Pode perguntar sobre comportamento, regulação emocional, estratégias caseiras ou quando procurar ajuda profissional.`,
    },
  ]);
  ...
  const send = async () => {
    ...
    try {
      ...
      setMsgs([
        ...
        {
          role: "ai",
          t: "Ops, tive um probleminha para pensar agora. Tente de novo em instantes!",
        },
      ]);
    } finally {
      setIsLoading(false);
  ...
  return (
    <Shell>
      <PageHeader
        icon={MessagesSquare}
        title="Terapeuta Brilha"
        subtitle="IA especializada em neurodesenvolvimento — orientação para a família"
      />
        ...
        <div className="flex-1 space-y-3 mb-4">
          {msgs.map((m, i) => (
            <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
              <div
                className={`max-w-[80%] rounded-2xl px-4 py-2.5 ${m.role === "user" ? "bg-primary text-primary-foreground" : "bg-secondary"}`}
              >
      ...
      <p className="text-xs text-muted-foreground text-center">
        A Terapeuta Brilha não substitui acompanhamento clínico — é um apoio entre consultas.
      </p>
    </Shell>
  );
}
