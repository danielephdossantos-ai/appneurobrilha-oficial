...
function RelatorioPremium() {
  ...
  if (loading)
    return (
      <Shell>
        <div className="p-10 text-center">Carregando análise premium...</div>
      </Shell>
    );
    ...
    return (
      <Shell>
        <PageHeader
          ...
          subtitle="Análise do Desenvolvimento"
        />
        <Card className="text-center py-10 max-w-xl mx-auto mt-10">
          <div className="text-5xl mb-4 text-primary mx-auto flex justify-center">
            <PenTool className="h-12 w-12" />
          </div>
          <h2 className="text-xl font-bold mb-2">Anamnese Necessária</h2>
          <p className="text-muted-foreground mb-6">
            A anamnese ainda não foi concluída para gerar o perfil fixo de desenvolvimento.
          </p>
          <button
      ...
      <div className="max-w-5xl mx-auto space-y-8 pb-20">
        ...
        <Card className="bg-gradient-to-br from-indigo-50 to-white border-indigo-100 shadow-xl overflow-hidden relative">
          <div className="absolute top-0 right-0 p-4">
            <div className="text-[10px] font-bold bg-indigo-600 text-white px-3 py-1 rounded-full uppercase tracking-widest shadow-sm">
              Perfil Fixo
            </div>
          </div>
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h3 className="text-2xl font-extrabold text-indigo-900 leading-tight">
                Perfil de Desenvolvimento Inicial
              </h3>
              <p className="text-indigo-600/70 text-sm font-bold">
                Diagnóstico base gerado via anamnese
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {report?.initialProfile.slice(0, 5).map((item, i) => (
              <div
                key={i}
                className="flex flex-col gap-2 p-5 bg-white/80 rounded-[2rem] border border-indigo-50 shadow-sm hover:shadow-md transition-all"
              >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <Card className="shadow-lg border-slate-100 p-8">
                        ...
                        return (
                          <div className="bg-white p-4 shadow-2xl rounded-2xl border border-slate-100">
                            ...
                            <p
                              ...
                              style={{ color: COLORS[data.status as keyof typeof COLORS] }}
                            >
            ...
            <div className="mt-6 flex flex-wrap gap-6 justify-center text-[10px] font-black uppercase tracking-widest text-slate-400">
              <div className="flex items-center gap-2">
                <div className="w-4 h-4 rounded-full bg-[#10b981] shadow-sm shadow-emerald-200" />{" "}
                Adequado
              </div>
              <div className="flex items-center gap-2">
                <div className="w-4 h-4 rounded-full bg-[#f59e0b] shadow-sm shadow-amber-200" /> Em
                desenvolvimento
              </div>
              <div className="flex items-center gap-2">
                <div className="w-4 h-4 rounded-full bg-[#ef4444] shadow-sm shadow-red-200" />{" "}
                Atenção
              </div>
            </div>
          </Card>
          ...
          <Card className="shadow-lg border-primary/20 bg-primary/[0.02] p-8">
            <div className="flex items-center justify-between mb-8">
              ...
              <div className="text-[10px] font-bold bg-primary text-white px-3 py-1 rounded-full uppercase tracking-widest shadow-sm">
                Uso do App
              </div>
            </div>
            <div className="space-y-6">
              {report?.evolution.map((ev, i) => (
                <div
                  key={i}
                  className="p-6 rounded-[2rem] bg-white border border-primary/10 flex items-start gap-5 shadow-sm hover:shadow-md transition-all"
                >
                  <div
            ...
            <div className="mt-10 p-5 bg-white rounded-2xl border-2 border-dashed border-primary/20 text-center">
              <p className="text-xs text-primary font-black uppercase tracking-tighter">
                Dados atualizados automaticamente com base no desempenho real.
              </p>
            </div>
          </Card>
        </div>
                  ...
                  >
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <span className="font-black text-primary text-sm">
                        {m.mastery_percentage || Math.round(m.success_rate * 100)}%
                      </span>
                    </div>
                    ...
                    <div className="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden">
                      <div
                        className={`h-full transition-all ${
                          (m.mastery_percentage || m.success_rate * 100) >= 80
                            ? "bg-success"
                            : (m.mastery_percentage || m.success_rate * 100) >= 50
                              ? "bg-warning"
                              : "bg-destructive"
                        }`}
          ...
          <Card className="border-emerald-100 bg-emerald-50/40 shadow-md p-8">
            ...
            <div className="grid grid-cols-1 gap-3">
              {report?.strengths.map((s, i) => (
                <div
                  key={i}
                  className="p-4 bg-white rounded-2xl border border-emerald-100 text-emerald-900 text-sm font-black shadow-sm flex items-center gap-3"
                >
          ...
          <Card className="border-amber-100 bg-amber-50/40 shadow-md p-8">
            ...
            <div className="grid grid-cols-1 gap-3">
              {report?.attentionPoints.map((s, i) => (
                <div
                  key={i}
                  className="p-4 bg-white rounded-2xl border border-amber-100 text-amber-900 text-sm font-black shadow-sm flex items-center gap-3"
                >
        ...
        <Card className="bg-slate-900 text-white border-slate-800 shadow-2xl p-10 rounded-[3rem]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {report?.recommendations.map((r, i) => (
              <div
                key={i}
                className="p-6 bg-white/5 hover:bg-white/10 transition-all rounded-[2rem] border border-white/10 flex flex-col gap-4 group"
              >
        ...
        <div className="p-10 bg-slate-100 rounded-[3rem] border-2 border-slate-200 shadow-inner">
          <div className="flex items-start gap-6">
            <div className="bg-white p-5 rounded-3xl text-slate-400 shadow-md shrink-0">
            ...
            <div className="space-y-4">
              ...
              <p className="text-sm text-slate-600 leading-relaxed font-bold">
                Este relatório é baseado em triagem inteligente e uso do app.{" "}
                <strong>Não substitui avaliação clínica profissional.</strong> O diagnóstico base é
                estável e reflete a anamnese inicial. A evolução dinâmica mostra o progresso da
                criança no NeuroBrilha Kids. Se houver dúvidas, recomendamos buscar avaliação com um
                profissional especializado (psicólogo, neurologista ou fonoaudiólogo).
              </p>
            </div>
          </div>
        </div>
