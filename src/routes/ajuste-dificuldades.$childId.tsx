  ...
  return (
    <Shell>
      <PageHeader
        emoji="🎚️"
        title="Ajuste fino"
        subtitle={`Sobrescrever o que o sistema deduziu sobre ${child.nome}`}
      />
              ...
              <div key={m.k}>
                <div className="flex justify-between items-baseline mb-1">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {NIVEL_DESC[val as 1 | 2 | 3 | 4].titulo}
                  </span>
                </div>
                <div className="grid grid-cols-4 gap-1.5">
                  {[1, 2, 3, 4].map((n) => (
                    <button
                      key={n}
                      onClick={() => setNiveis({ ...niveis, [m.k]: n as any })}
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {NIVEL_DESC[val as 1 | 2 | 3 | 4].desc}
                </p>
              </div>
            );
          })}
      ...
      <Card className="mb-4">
        ...
        />
        <p className="text-xs text-muted-foreground mt-1">
          Duração de cada sessão antes da pausa ativa.
        </p>
      </Card>
      ...
      <button
        onClick={salvar}
        className="btn-tap rounded-xl bg-primary text-primary-foreground px-6 py-3 font-bold flex items-center gap-2"
      >
