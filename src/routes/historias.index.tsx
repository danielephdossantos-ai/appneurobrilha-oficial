...
export const Route = createFileRoute("/historias/")({
  ...
  head: () => ({
    meta: [
      { title: "Historias — NeuroBrilha Kids" },
      { name: "description", content: "Biblioteca de historias para alfabetizacao infantil com leitura guiada e narracao." },
    ],
  }),
});
...
function StoryCover({ coverImage, theme }: { coverImage: string | null | undefined; theme: string }) {
  const isRealImage =
    coverImage &&
    (coverImage.startsWith("data:image/png") ||
      ...
      <div className="relative w-full h-40 rounded-2xl overflow-hidden">
        ...
        <div className="absolute bottom-2 right-2">
          <div className="flex items-center gap-1 bg-black/30 backdrop-blur-sm rounded-full px-2 py-0.5">
            ...
            <span className="text-white/80 text-[9px] font-bold">IA</span>
          </div>
        </div>
      </div>
    );
    ...
    >
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-gray-500 font-medium text-sm max-w-xs">
          A inteligencia artificial esta escrevendo historias com ilustracoes para cada tema. Isso leva cerca de 1 minuto.
        </p>
      </div>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {sorted.map((s: any, i: number) => {
              ...
              const cover = s.coverImage ?? s.cover_image;
              const ageMin = s.ageMin ?? s.age_min;
              const ageMax = s.ageMax ?? s.age_max;
              return (
                <motion.div
                  key={s.id}
                ...
                >
                  ...
                  >
                    <StoryCover coverImage={cover} theme={s.theme} />
