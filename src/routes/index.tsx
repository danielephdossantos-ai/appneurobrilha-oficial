  ...
  return (
    <Shell>
      ...
      <div className="md:hidden flex flex-col gap-3 -mt-2">
        {/* Hero compacto */}
        <div
          className="relative rounded-[2rem] overflow-hidden border-4 border-white shadow-glow px-4 pt-4 pb-3"
          style={{
            background: "radial-gradient(circle at 50% 110%, #1a3a8c 0%, #0d1f55 55%, #050a2c 100%)",
          }}
        >
          <Stars />
          {/* Aurora */}
          <div className="absolute inset-0 pointer-events-none opacity-50"
            style={{ background: "radial-gradient(ellipse at 20% 10%, rgba(155,108,255,0.35), transparent 50%), radial-gradient(ellipse at 80% 15%, rgba(76,158,255,0.35), transparent 50%)" }}
          />
          ...
          <div className="relative z-10 flex items-center gap-3">
            ...
            >
              <div className="relative">
                <div className="absolute inset-0 -m-3 rounded-full"
                  style={{ background: "radial-gradient(circle, rgba(255,201,60,0.5), transparent 70%)", filter: "blur(8px)" }}
                />
            ...
            <div className="flex-1 min-w-0">
              <div className="text-white/60 text-[10px] font-black uppercase tracking-[0.25em]">Bem-vindo à</div>
              <h1 className="text-white text-xl font-black leading-tight">
                Cidade do <span className="text-[#FFC93C]">Pip</span>
              </h1>
              <div className="mt-1 bg-white/90 backdrop-blur px-3 py-1.5 rounded-xl border border-[#FFC93C]/60 inline-block">
                <p className="text-[#0d1f55] font-black text-xs leading-tight">
                  {activeChild
                    ? `Oi ${activeChild.nome}! Pra onde vamos? 🚀`
                    : "Escolha um aventureiro! ✨"}
                </p>
              </div>
            </div>

            {/* Avatar criança ativo */}
            {activeChild && (
              <div className="shrink-0 h-10 w-10 rounded-2xl bg-white/20 border-2 border-white/40 grid place-items-center text-2xl">
                {activeChild.avatar}
              </div>
            )}
          </div>
        ...
        {/* Grade 2×3 — todas as categorias visíveis */}
          ...
          {destinos.map((d, i) => (
            <Link key={d.to} to={d.to}>
              <motion.div
                ...
                whileTap={{ scale: 0.95 }}
                className={cn(
                  "relative rounded-[1.5rem] overflow-hidden border-[3px] border-white/90 cursor-pointer",
                  "bg-gradient-to-br", d.gradient,
                )}
                style={{ boxShadow: `0 6px 0 rgba(0,0,0,0.22), 0 0 24px ${d.glow}` }}
              >
                {/* Ícone canto */}
                <d.icon className="absolute top-2.5 right-2.5 text-white/35 w-4 h-4" />

                {/* Ilustração compacta */}
                <div className="h-20 flex items-end justify-center px-3 pt-3">
                ...
                {/* Label */}
                <div className="mx-2 mb-2 bg-white/95 rounded-xl px-2 py-2 text-center">
                  <div className="text-sm font-black text-[#0d1f55] leading-tight">{d.title}</div>
                  <div className="text-[8px] font-black uppercase tracking-wider text-[#0d1f55]/55 mt-0.5">{d.subtitle}</div>
                </div>
              </motion.div>
            </Link>
          ))}
        ...
        {/* Seletor de criança — compacto */}
        <div className="mt-1">
          <div className="flex items-center gap-2 mb-2">
            <Sparkles className="text-[#FFC93C] h-4 w-4" />
            <span className="text-xs font-black uppercase tracking-[0.2em] text-[#0d1f55]/60">Quem vai brilhar?</span>
          </div>
          <div className="flex gap-2 flex-wrap">
            {allChildren.map((c: any) => (
              <button
                ...
                className={cn(
                  "flex items-center gap-2 rounded-2xl px-3 py-2 border-3 border-[3px] transition-all active:scale-95 text-left",
                  c.id === activeChild?.id
                    ? "bg-gradient-to-br from-[#4C9EFF] to-[#1E5FCC] border-[#FFC93C] text-white shadow-lg"
                    : "bg-white border-white shadow-md",
                )}
                style={c.id === activeChild?.id ? { boxShadow: "0 4px 0 rgba(0,0,0,0.12), 0 0 20px rgba(76,158,255,0.4)" } : {}}
              >
                <div className="h-8 w-8 rounded-xl bg-white/25 grid place-items-center text-lg">{c.avatar}</div>
                <div>
                  <div className="font-black text-xs leading-tight">{c.nome}</div>
                  <div className={cn("text-[9px] font-bold uppercase tracking-wider", c.id === activeChild?.id ? "text-white/70" : "text-[#0d1f55]/55")}>
                    {c.idade}a
                  </div>
                </div>
              </button>
            ...
            <Link
              to="/anamnese/$childId"
              params={{ childId: "nova" }}
              className="flex items-center gap-2 rounded-2xl px-3 py-2 border-[3px] border-dashed border-[#4C9EFF]/40 bg-white/70 active:scale-95 transition-all"
            >
              <div className="h-8 w-8 rounded-xl bg-[#4C9EFF] text-white grid place-items-center">
                <Plus className="h-4 w-4" strokeWidth={3} />
              </div>
              <span className="font-black text-[#0d1f55] uppercase tracking-wider text-[9px]">Nova</span>
            </Link>
          </div>
        </div>
        ...
        {activeChild && !activeChild.anamnese_completa && (
          <Link to="/anamnese/$childId" params={{ childId: activeChild.id }}>
            <div className="rounded-2xl p-4 bg-gradient-to-r from-[#FFC93C] to-[#FF8A4C] border-[3px] border-white shadow-lg flex items-center gap-3 active:scale-[0.98] transition-transform">
              <div className="text-3xl animate-bounce">⚠️</div>
              <div>
                <div className="font-black text-[#0d1f55] text-sm leading-tight">Anamnese de {activeChild.nome} pendente</div>
                <div className="text-[10px] font-black uppercase tracking-widest text-[#0d1f55]/70">Toque para começar</div>
              </div>
            </div>
          </Link>
        )}
      ...
      <div className="hidden md:block">
        <div
          className="relative w-full min-h-[88vh] -mt-6 rounded-[3rem] overflow-hidden border-4 border-white shadow-glow"
        ...
        >
          ...
          <div className="relative z-10 flex items-center justify-between p-6 md:p-10">
            <div className="flex items-center gap-3">
              <motion.div animate={{ rotate: [0, 8, -8, 0] }} transition={{ duration: 4, repeat: Infinity }} className="text-4xl">
            ...
            {activeChild && (
              <div className="flex items-center gap-3 bg-white/10 backdrop-blur-md px-5 py-3 rounded-full border-2 border-white/20">
          ...
          <div className="relative z-20 flex flex-col items-center justify-center mt-0 mb-4 md:mb-12">
            ...
            >
              <div className="absolute inset-0 -m-12 rounded-full"
                style={{ background: "radial-gradient(circle, rgba(255,201,60,0.4), transparent 65%)", filter: "blur(20px)" }}
              />
              <div className="relative"><KidLiveMascot size="xl" emotion="happy" /></div>
            </motion.div>
            <motion.div
          ...
          <div className="relative z-10 px-4 md:px-10 pb-10">
            <div className="grid grid-cols-3 gap-5 md:gap-7 max-w-6xl mx-auto">
              {destinos.map((d, i) => (
                <Link key={d.to} to={d.to} className="group">
                  <motion.div
                    ...
                    className={cn("relative rounded-[2rem] p-5 md:p-6 cursor-pointer overflow-hidden border-4 border-white/90 bg-gradient-to-br", d.gradient)}
                    style={{ boxShadow: `0 12px 0 rgba(0,0,0,0.25), 0 0 40px ${d.glow}` }}
                  >
                    ...
                    <div className="relative z-10 bg-white/95 rounded-2xl p-3 text-center">
                      <div className="text-xl md:text-2xl font-black text-[#0d1f55] leading-tight">{d.title}</div>
                      <div className="text-[10px] md:text-xs font-black uppercase tracking-widest text-[#0d1f55]/60 mt-0.5">{d.subtitle}</div>
                    </div>
                  </motion.div>
                </Link>
              ))}
        ...
        <div className="mt-10">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {allChildren.map((c: any) => (
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="h-14 w-14 rounded-2xl bg-white/30 grid place-items-center text-3xl">{c.avatar}</div>
            ...
            >
              <div className="w-9 h-9 rounded-full bg-[#4C9EFF] text-white grid place-items-center"><Plus strokeWidth={3} /></div>
            ...
            <Link to="/anamnese/$childId" params={{ childId: activeChild.id }}>
              <div className="mt-6 rounded-3xl p-5 bg-gradient-to-r from-[#FFC93C] to-[#FF8A4C] border-4 border-white shadow-xl flex items-center gap-4 hover:scale-[1.01] transition-transform">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-[11px] font-black uppercase tracking-widest text-[#0d1f55]/70">Toque para começar</div>
                </div>
              </div>
            </Link>
          )}
