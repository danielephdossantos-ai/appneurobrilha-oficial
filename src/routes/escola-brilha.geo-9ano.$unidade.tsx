  ...
  return (
    <PlayerGeoV2
      ...
      onConcluir={() => {
        ...
        try {
          const raw = localStorage.getItem(`eb.geo9ano.progresso.${localStorage.getItem("neurobrilha:activeChildId") || "sem-crianca"}`);
          const arr: string[] = raw ? JSON.parse(raw) : [];
          if (!arr.includes(unidade)) {
            arr.push(unidade);
            localStorage.setItem(`eb.geo9ano.progresso.${localStorage.getItem("neurobrilha:activeChildId") || "sem-crianca"}`, JSON.stringify(arr));
          }
          cicloCompleto = UNIDADES_9ANO.every((item) => arr.includes(item.slug));
        } catch {
          /* ignore */
        }
        if (ehFinal && cicloCompleto) {
          ...
          navigate({ to: "/escola-brilha/trilha-fund2/$serie/$disc", params: { serie: "9ano", disc: "geografia" } });
        } else if (proxima) {
