...
import { AlertCircle, Coffee, ChevronRight, Brain, Mic, Dumbbell, Eye, BookOpen, Sparkles, Zap } from "lucide-react";
...
const GROUP_CONFIG: Record<string, { icon: ReactNode; color: string; bg: string; border: string; accent: string }> = {
  "Fono Clínico": {
...
function Treino() {
  ...
  useEffect(() => {
    ...
    const STALE_KEYS = [
      "neuroTreino:world","neuroTreino:trilha","neuroTreino:trail","neuroTreino:lastRoute",
      "neuroTreino:redirect","neuro-treino:world","neuro-treino:trilha","neuro-treino:redirect",
      "currentWorld","lastJourneyRoute",
    ];
    try { STALE_KEYS.forEach((k) => { window.localStorage.removeItem(k); window.sessionStorage.removeItem(k); }); }
    catch { /* ignore */ }
  }, []);
    ...
    <Shell className="pb-8">
          ...
          <div className="flex items-start justify-between mb-3">
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-xs text-slate-400 font-bold uppercase tracking-widest">Consultório Digital</span>
              </div>
            ...
            <div className="bg-white/8 rounded-2xl px-3 py-2.5 border border-white/10">
              ...
              <div className="flex items-end gap-1.5">
                ...
                <div className="flex-1 h-1.5 rounded-full bg-white/10 mb-1 overflow-hidden">
                  <div className="h-full rounded-full bg-emerald-400 transition-all" style={{ width: `${precisao}%` }} />
                </div>
              </div>
            </div>
            <div className="bg-white/8 rounded-2xl px-3 py-2.5 border border-white/10">
              ...
              <div className="flex items-end gap-1.5">
                ...
                <div className="flex-1 h-1.5 rounded-full bg-white/10 mb-1 overflow-hidden">
                  <div className="h-full rounded-full bg-sky-400 transition-all" style={{ width: `${100 - fadiga}%` }} />
                </div>
              </div>
            </div>
          </div>
      ...
      {adjustment.suggestBreak && (
        <div className="mb-5 bg-amber-50 dark:bg-amber-950/30 rounded-2xl border-2 border-amber-200 dark:border-amber-800 px-4 py-3 flex items-center gap-3 shadow-sm">
          ...
          <div className="flex-1 min-w-0">
            <div className="font-bold text-amber-900 dark:text-amber-200 text-sm">Que tal uma pausinha?</div>
            <div className="text-xs text-amber-700 dark:text-amber-400">Detectei sinais de cansaço. Respira fundo.</div>
          </div>
        </div>
      )}
      ...
      <div className="space-y-5">
        {GRUPOS.map((g) => {
          ...
          return (
            <section key={g.nome} className={`rounded-3xl border ${cfg?.border ?? "border-border"} overflow-hidden shadow-sm`}>
              {/* Cabeçalho do grupo */}
              <div className={`bg-gradient-to-r ${cfg?.bg ?? "from-muted to-muted/50"} px-4 py-3 flex items-center gap-2 border-b ${cfg?.border ?? "border-border"}`}>
                <div className={`${cfg?.color ?? "text-primary"}`}>{cfg?.icon ?? <Sparkles className="h-5 w-5" />}</div>
                <div>
                  <h2 className={`font-black text-sm ${cfg?.color ?? "text-foreground"} uppercase tracking-wider`}>{g.nome}</h2>
                  <div className="text-xs text-muted-foreground">{g.slugs.length} atividades · {g.slugs.length * 30} variações</div>
                </div>
              </div>
                    ...
                    >
                      ...
                      {varCount > 0 && (
                        <div className="absolute top-2 right-2">
                          <div className={`w-5 h-5 rounded-full ${cfg?.accent ?? "bg-primary"} flex items-center justify-center`}>
