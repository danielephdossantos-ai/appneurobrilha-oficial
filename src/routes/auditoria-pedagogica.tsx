    ...
    queryFn: async () => {
      try {
        ...
        return data.map((item: any) => ({
          ...
          mechanic:
            item.completion_data?.mechanic || (item.completion_data as any)?.type || "selection",
  ...
  return (
    <Shell>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-2xl font-black">
                {Math.round(
                  (entries.reduce((acc, curr) => acc + curr.accuracy_rate, 0) / entries.length) *
                    100,
                )}
                %
              </div>
        ...
        <Card className="bg-sky/5 border-sky/20">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-2xl font-black">
                {new Set(entries.map((e) => e.bncc_code)).size}
              </div>
            </div>
          </div>
        </Card>
        <Card className="bg-coral/5 border-coral/20">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-2xl font-black">
                {new Set(entries.map((e) => e.neuro_profile)).size}
              </div>
            </div>
          </div>
        </Card>
      ...
      <div className="mt-8 p-6 bg-warning/10 rounded-3xl border-2 border-dashed border-warning/30 flex items-start gap-4">
        ...
        <div>
          <h4 className="font-black text-warning-foreground uppercase tracking-tight">
            Nota de Auditoria
          </h4>
          <p className="text-sm text-muted-foreground mt-1 font-medium">
            Estes dados são gerados em tempo real pelo motor adaptativo. A "Validação Humana"
            permite identificar se o conteúdo gerado mantém a coerência pedagógica mesmo com a alta
