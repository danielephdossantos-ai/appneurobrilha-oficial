...
async function carregarPorDisciplina() {
    ...
    for (const [disc, lista] of porDisc) {
        ...
        for (const h of lista)
            if (!setOrd.has(h.codigo))
                ordenada.push(h);
        porDisc.set(disc, ordenada);
    }
    return porDisc;
}
async function carregarProgresso(childId) {
    const { data } = await supabase
    ...
    const mapa = new Map();
    for (const r of (data ?? []))
        mapa.set(r.codigo_bncc, r);
    return mapa;
}
function statusDaLinha(p) {
    if (!p)
        return "pendente";
    if (p.concluida)
        return "concluida";
    if ((p.percentual ?? 0) > 0 || p.ultima_visita_em)
        return "em_andamento";
    return "pendente";
}
// =====================================================================
...
// =====================================================================
function montarTrilha(disciplina, nos, prog) {
    const enriquecidos = nos.map((h, i) => {
    ...
    const continuarDe = naoConcluidasVisitadas[0] ??
...
export const TrilhasAprendizagem = {
    ...
    async todas(childId) {
        ...
        const out = [];
        for (const [disc, nos] of porDisc)
            out.push(montarTrilha(disc, nos, prog));
        out.sort((a, b) => a.disciplina.localeCompare(b.disciplina));
        return out;
    },
    /** Trilha específica de uma disciplina. */
    async da(childId, disciplina) {
    ...
    async continuarDe(childId, disciplina) {
    ...
    async proximaSugerida(childId, disciplina) {
        const t = await TrilhasAprendizagem.da(childId, disciplina);
