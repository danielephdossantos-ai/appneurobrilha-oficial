...
const KEY = (childId) => `eb_conquistas_v1:${childId}`;
...
function salvar(childId, estado) {
    if (typeof window === "undefined")
        return;
    try {
...
function diasConsecutivos(dias) {
    if (dias.length === 0)
        return 0;
...
export const ConquistasEducacionais = {
    ...
    listar(childId) {
        return carregar(childId).conquistadas.slice().sort((a, b) => a.conquistadaEm.localeCompare(b.conquistadaEm));
    },
