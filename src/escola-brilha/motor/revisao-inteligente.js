 ...
 */
import { CONTEXTOS, exemplosParaHabilidade } from "./exemplos-contexto";
import { selecionarAtividadesDetalhado } from "../atividades";
const STORAGE_KEY = "eb_revisao_hist_v1";
function chaveHist(childId, codigoBncc) {
...
function rotativa(itens, rodada, quantos) {
    if (itens.length === 0)
        return [];
    const out = [];
 ...
 */
export function gerarPacoteRevisao(childId, codigoBncc, opts = {}) {
    ...
    const registro = hist[chave] ??
        { rodada: 0, contextosUsados: [], idsVariantesUsadas: [], atualizadoEm: new Date().toISOString() };
    ...
    const atividades = selecionarAtividadesDetalhado(codigo, qAt);
    const desafios = rotativa(DESAFIOS_BASE, rodada, qDf).map((d, i) => `${d} — usando o contexto de ${contextos[i % contextos.length]} (rodada ${rodada}).`);
    ...
    const contextosUsadosAtual = Array.from(new Set([...registro.contextosUsados, ...contextos]));
    const contextosUsados = contextosUsadosAtual.length >= CONTEXTOS.length ? contextos.slice() : contextosUsadosAtual;
    hist[chave] = {
        rodada,
        contextosUsados,
        idsVariantesUsadas: Array.from(new Set([...registro.idsVariantesUsadas, ...idsVariantes])).slice(-200),
...
export function reiniciarHistoricoRevisao(childId, codigoBncc) {
    const hist = ler();
    if (!codigoBncc) {
