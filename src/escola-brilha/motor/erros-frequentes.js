...
function prefixoDisciplina(codigoBncc) {
    const m = codigoBncc.toUpperCase().match(/^EF\d{2}([A-Z]{2})/);
    if (m)
        return m[1];
    const alt = codigoBncc.toUpperCase().match(/^EI/);
    if (alt)
        return "LP"; // Educação Infantil aproxima linguagem
    return "MA";
}
export function catalogoDaHabilidade(codigoBncc) {
...
export function detectarErro(codigoBncc, sinal) {
    ...
    if (resposta) {
        ...
        for (const e of cat.entradas) {
            const hits = e.sinais.reduce((acc, s) => acc + (resposta.includes(normaliza(s)) ? 1 : 0), 0);
    ...
    const respostaLenta = (sinal.tempoRespostaMs ?? 0) > 45000;
