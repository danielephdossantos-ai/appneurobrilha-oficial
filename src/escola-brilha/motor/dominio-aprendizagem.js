...
function componentDesafios(s) {
    if (s.desafiosTentados <= 0)
        return 0;
    return clamp01(s.desafiosAcertados / s.desafiosTentados);
}
function componentAvaliacoesPosteriores(s) {
    if (s.sessoesPosteriores <= 0)
        return 0;
...
export function calcularDominio(sinais) {
    const c = {
    ...
    const score = clamp100(c.acertos * PESOS.acertos +
        c.erros * PESOS.erros +
    ...
    if (sinais.sessoesPosteriores > 0)
        evidencias.push(`${sinais.sessoesPosteriores} sessão(ões) posterior(es) com média ${Math.round(sinais.desempenhoSessoesPosteriores)}%`);
    const proximoPasso = nivel === "dominada"
...
async function carregarProgresso(childId, codigoBncc) {
    const { data } = await supabase
        .from("escola_progresso")
        .select("acertos, erros, tentativas, revisoes_realizadas, tempo_medio_segundos, concluida, updated_at, historico_evolucao")
        .eq("child_id", childId)
...
function extrairSinais(p, tempoEsperado = TEMPO_ESPERADO_PADRAO_S) {
    const historico = p?.historico_evolucao ?? [];
    const desafios = historico.filter((h) => h.tipo === "desafio");
    const avaliacoesPosteriores = historico.filter((h) => h.tipo === "avaliacao" || h.tipo === "revisao");
    ...
    const desempenhoMedio = sessoesPosteriores > 0
...
async function persistirNivel(childId, codigoBncc, nivel) {
    // Mapeia para o enum atual do banco (compatibilidade retroativa).
    const nivelBanco = nivel === "nivel_3_consolidacao" ? "parcialmente_dominada" : // Mapeamento para legado
...
export const DominioAprendizagem = {
    ...
    async avaliar(childId, codigoBncc, tempoEsperadoSegundos) {
     ...
     */
    async registrarInteracao(childId, codigoBncc, _interacao) {
     ...
     * "Continue praticando".
     */
    concluiuNaoImplicaDominio(detalhe) {
