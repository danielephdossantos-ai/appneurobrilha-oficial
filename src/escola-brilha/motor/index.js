...
import { getMissaoOficial, hasMissaoOficial, listMissoesOficiais, listMissoesPorAno, listMissoesPorDisciplina, totalMissoesOficiais, } from "../biblioteca-oficial";
import { proximaHabilidade, invalidarCacheSequencia } from "../bncc-sequencia";
import { proximaMissao, metaDaAula, metaDoCodigo, estimarTempo, dificuldadeDe, } from "../proxima-missao";
import { supabase } from "@/integrations/supabase/client";
import { resolverMissao, parseBNCC, MENSAGEM_MISSAO_EM_CONSTRUCAO } from "./resolver";
import { selecionarAtividades, selecionarAtividadesDetalhado, CATALOGO_ATIVIDADES, TODOS_TIPOS, } from "../atividades";
import { AdaptacaoMissao, planejarAdaptacao, } from "./adaptacao-missao";
import { ExemplosContexto, exemplosParaHabilidade, CONTEXTOS, } from "./exemplos-contexto";
import { RevisaoInteligente, gerarPacoteRevisao, reiniciarHistoricoRevisao, } from "./revisao-inteligente";
import { SistemaMotivacao, proximaMensagem, reiniciarMotivacao, } from "./motivacao";
import { AvaliacaoContinua, } from "./avaliacao-continua";
import { BancoErrosFrequentes, catalogoDaHabilidade, detectarErro, } from "./erros-frequentes";
import { TrilhasAprendizagem, } from "./trilhas";
import { ConquistasEducacionais, CATALOGO_CONQUISTAS, } from "./conquistas-educacionais";
import { BibliotecaNacional, } from "./biblioteca-nacional";
import { DominioAprendizagem, calcularDominio, } from "./dominio-aprendizagem";
// =====================================================================
...
const Missoes = {
    ...
    listarPorDisciplina(disciplina) {
...
const AdaptacaoPorIdade = {
    calcular(perfil) {
        ...
        const infantil = idade > 0 && idade <= 6 ||
...
const Revisao = {
     ...
     */
    async registrarConclusao(childId, codigoBncc, desempenho, opts = {}) {
        const { data, error } = await supabase.rpc("registrar_conclusao_aula", {
            ...
            _dificuldades: (opts.dificuldades ?? []),
        });
        if (error) {
...
// =====================================================================
const Progresso = {
    async carregar(childId, codigoBncc) {
        const { data } = await supabase
            .from("escola_progresso")
            .select("bloco_atual, concluida, percentual, nivel_dominio, tentativas, acertos, erros, sessoes_dominadas_consecutivas, facilidade, dificuldades, revisoes_realizadas, evolucao_delta, historico_evolucao, tempo_medio_segundos, tempo_estudado_segundos")
            .eq("child_id", childId)
            .eq("codigo_bncc", codigoBncc)
            .maybeSingle();
        return data ?? null;
    },
    async podeAvancar(childId, codigoBncc) {
...
// =====================================================================
const Conquistas = {
    async registrar(childId, codigo, tipo) {
        try {
