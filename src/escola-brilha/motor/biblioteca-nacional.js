...
// =====================================================================
function validarEstrutura(m) {
    const p = [];
    const add = (campo, mensagem) => p.push({ categoria: "estrutura", campo, mensagem });
    if (!isStr(m.codigo))
        add("codigo", "código BNCC obrigatório");
    if (!isStr(m.etapa))
        add("etapa", "etapa escolar obrigatória");
    if (!isStr(m.ano))
        add("ano", "ano escolar obrigatório");
    if (!isStr(m.disciplina))
        add("disciplina", "disciplina obrigatória");
    if (!isStr(m.unidadeTematica))
        add("unidadeTematica", "unidade temática BNCC obrigatória");
    if (!isStr(m.objetoConhecimento))
        add("objetoConhecimento", "objeto de conhecimento BNCC obrigatório");
    ...
    if (!isArr(m.exemplos))
        add("exemplos", "pelo menos 1 exemplo resolvido");
    else {
    ...
    if (!isArr(m.atividades))
        add("atividades", "pelo menos 1 atividade prática");
    else {
    ...
    if (!isArr(m.desafios))
        add("desafios", "pelo menos 1 desafio");
    else {
    ...
    if (!isArr(m.quiz, 3))
        add("quiz", "mínimo 3 perguntas");
    else {
        m.quiz.forEach((q, i) => {
            if (!isStr(q?.pergunta))
                add(`quiz[${i}].pergunta`, "pergunta obrigatória");
            if (!isArr(q?.opcoes, 2))
    ...
    if (!isArr(m.adaptacoes))
        add("adaptacoes", "adaptações pedagógicas obrigatórias");
    if (!isArr(m.objetivos))
        add("objetivos", "pelo menos 1 objetivo de aprendizagem");
    if (!isArr(m.competencias))
        add("competencias", "pelo menos 1 competência BNCC");
    if (!isObj(m.meta) || !isStr(m.meta.versao) || !isStr(m.meta.atualizadoEm))
        add("meta", "versão + atualizadoEm obrigatórios");
    return p;
}
async function buscarHabilidadeBNCC(codigo) {
...
async function validarVinculoBNCC(m) {
    const problemas = [];
...
// 3. Ordem pedagógica
// =====================================================================
function validarOrdemPedagogica(m) {
    const problemas = [];
...
function validarConsistencia(m, hab) {
    const p = [];
    if (!hab)
        return p; // já reportado em vínculo
    ...
    const etapaEsperada = prefixo === "EI" ? "Educação Infantil" :
...
// =====================================================================
function validarIntegridade(m) {
    const problemas = [];
    const avisos = [];
    const checarTexto = (campo, v) => {
    ...
    if (isArr(m.quiz)) {
        m.quiz.forEach((q, i) => {
            ...
            if (isArr(opcoes, 2) && isNum(correta)) {
                if (correta < 0 || correta >= opcoes.length) {
                    problemas.push({
                        categoria: "integridade",
                        campo: `quiz[${i}].correta`,
                        mensagem: `índice ${correta} fora do intervalo (0..${opcoes.length - 1})`,
                    });
                }
            }
        });
    ...
    const publicos = new Set((m.adaptacoes ?? []).map((a) => normalizar(a?.publico)).filter((s) => s.length > 0));
    ...
    async cobertura() {
        ...
        for (;;) {
            ...
            for (const row of data) {
                if (hasMissaoOficial(row.codigo_bncc))
                    publicadas++;
            }
