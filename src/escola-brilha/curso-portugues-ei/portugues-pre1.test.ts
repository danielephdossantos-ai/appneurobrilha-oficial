...
describe("Português EI — Pré I", () => {
  const aulas = cursoPre1.unidades.flatMap((unidade) => unidade.aulas);
  ...
  it("usa quatro imagens e uma resposta correta nas escolhas", () => {
    ...
    expect(escolhas).toHaveLength(23);
  ...
  it("não ensina o nome da letra como som nem mistura palavra e imagem", () => {
    const falas = aulas
      .flatMap((aula) => aula.momentos)
      .map((momento) => JSON.stringify(momento))
      .join(" ");
    expect(falas).not.toMatch(/som (CÊ|GÊ)/i);
    expect(falas).not.toContain("sol (peixe)");
  });
});
