...
// ============ FASE 8 · Semana 1 — Prever ============
function PreverHistoriaBloco({
  m,
  idx,
  cor,
  onOk,
}: {
  m: Extract<MomentoEI, { tipo: "preverHistoria" }>;
  idx: number;
  cor: string;
  onOk: () => void;
}) {
  ...
  const escolher = (nome: string) => {
    const alvo = m.opcoes.find((o) => o.nome === nome);
    if (!alvo) return;
    setEscolhida(nome);
    if (alvo.correta) {
      setOk(true);
  ...
  return (
    <CardScreen cor={cor}>
      <TituloMomento n={idx + 1} texto={m.titulo ?? "O que vai acontecer?"} cor={cor} />
      ...
      {contouTudo && (
        <>
          ...
          <div className="mt-4 grid grid-cols-3 gap-3 max-w-lg mx-auto w-full">
            {opcoes.map((o) => {
              const isEscolhida = escolhida === o.nome;
              const acertou = ok && o.correta;
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    stopSpeaking();
                    speakChunked(o.nome.toLowerCase(), { rate: 0.95, pitch: 1.1 });
                    setTimeout(() => escolher(o.nome), 500);
                  }}
                  className={`rounded-2xl border-4 p-2 shadow-md active:scale-95 transition ${
                    acertou
                ...
                >
                  <img
                    ...
                    className="w-full h-24 object-contain"
                  />
                  <div className="mt-1 text-[11px] font-black text-slate-800 uppercase text-center">
                    {o.nome}
                  </div>
                </button>
              );
      ...
      {!contouTudo && cenaAtual === 0 && (
        <p className="mt-3 text-center text-xs text-white/80">
          Toque ▶ para ouvir a história antes de escolher.
        </p>
      )}
...
function ResumirFraseBloco({
  m,
  idx,
  cor,
  onOk,
}: {
  m: Extract<MomentoEI, { tipo: "resumirFrase" }>;
  idx: number;
  cor: string;
  onOk: () => void;
}) {
  ...
  const opcoes = useMemo(() => {
    const seed = m.pergunta.length;
    return [...m.opcoes]
      .map((o, i) => ({ o, k: (o.frase.charCodeAt(0) + i * 17 + seed) % 97 }))
      .sort((a, b) => a.k - b.k)
      .map((x) => x.o);
  }, [m.opcoes, m.pergunta]);
  ...
  return (
    <CardScreen cor={cor}>
      <TituloMomento n={idx + 1} texto={m.titulo ?? "Conta em uma frase!"} cor={cor} />
            ...
            {opcoes.map((o, i) => {
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    stopSpeaking();
                    speakChunked(o.frase, { rate: 0.92, pitch: 1.05 });
                    setTimeout(() => escolher(i), 600);
                  }}
                  className={`rounded-2xl border-4 px-4 py-3 shadow-md active:scale-95 transition text-left ${
                    acertou
                      ? "border-green-400 bg-green-50"
                      : errou
                        ? "border-red-400 bg-red-50"
