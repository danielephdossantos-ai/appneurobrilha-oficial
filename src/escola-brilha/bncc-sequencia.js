...
function componenteDoCodigo(codigo) {
    const m = codigo.match(/^(EI|EF|EM)\d{2,4}([A-Z]{2,3})\d+/);
    return m?.[2] ?? "";
}
function unidadeTematica(codigo, disciplina) {
    const comp = componenteDoCodigo(codigo);
    const num = parseInt(codigo.match(/(\d+)$/)?.[1] ?? "0", 10);
    if (comp === "LP") {
        if (codigo.startsWith("EI"))
            return "Escuta, fala, pensamento e imaginação";
        if (num <= 10)
            return "Leitura / Escuta";
        if (num <= 20)
            return "Produção de textos";
        if (num <= 25)
            return "Oralidade";
    ...
    if (comp === "CI") {
        if (num <= 5)
            return "Matéria e Energia";
        if (num <= 10)
            return "Vida e Evolução";
        return "Terra e Universo";
    }
    if (comp === "HI")
        return "Mundos pessoais, sociais e históricos";
    if (comp === "GE")
        return "O sujeito, o lugar e o mundo";
    if (comp === "AR")
        return "Linguagens artísticas";
    if (comp === "EF" && codigo.startsWith("EF"))
        return "Práticas corporais";
    if (comp === "LI")
        return "Oralidade / Leitura / Escrita";
    if (comp === "ER")
        return "Identidades, alteridades e crenças";
    if (comp === "EF")
        return "O eu, o outro e o nós";
    if (comp === "TS")
        return "Traços, sons, cores e formas";
    if (comp === "ET")
        return "Escuta, fala, pensamento e imaginação";
    if (comp === "CG")
        return "Corpo, gestos e movimentos";
    if (comp === "EO")
        return "Espaços, tempos, quantidades, relações e transformações";
    return disciplina || "Componente Curricular";
}
// Cache em memória para não bater no banco a cada aula concluída.
let cacheSequencia = null;
let cachePromise = null;
async function carregarSequencia() {
    if (cacheSequencia)
        return cacheSequencia;
    if (cachePromise)
        return cachePromise;
    cachePromise = (async () => {
        ...
        for (const h of linhas) {
            if (!porDisc.has(h.disciplina))
                porDisc.set(h.disciplina, []);
 ...
 */
export async function proximaHabilidade(codigo) {
