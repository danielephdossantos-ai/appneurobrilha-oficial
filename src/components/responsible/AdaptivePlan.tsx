    ...
    <Card className="border-none shadow-sm bg-primary/5">
      ...
      <CardContent className="space-y-4">
        {recommendations.map((rec) => (
          <div
            key={rec.id}
            className="bg-white p-4 rounded-2xl shadow-sm border border-primary/10 flex gap-4"
          >
            <div className="p-3 bg-slate-50 rounded-xl self-start">{getIcon(rec.type)}</div>
            <div className="flex-1 space-y-2">
              ...
              <Button
                variant="ghost"
                className="h-8 text-[11px] font-bold text-primary p-0 hover:bg-transparent hover:text-primary/80"
              >
