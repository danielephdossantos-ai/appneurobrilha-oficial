    ...
    <Card className="bg-white border-slate-200 overflow-hidden shadow-sm">
      <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-gradient-to-r from-indigo-50/50 to-transparent">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Transforme provas em missões de aprendizado.</p>
        </div>
        <button 
        ...
        {isAdding && (
          <div className="bg-indigo-50/50 p-5 rounded-2xl border-2 border-dashed border-indigo-200 space-y-4 animate-in fade-in slide-in-from-top-4 duration-300">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-indigo-700 uppercase ml-1">Data da Prova</label>
                <input
                  type="date"
                  className="w-full px-4 py-2.5 rounded-xl border border-indigo-100 focus:ring-2 focus:ring-indigo-500 outline-none text-sm bg-white"
                  value={newDate}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-indigo-700 uppercase ml-1">Observações (Opcional)</label>
              <textarea
              ...
              <div key={mission.id} className="bg-white border border-slate-200 rounded-3xl p-5 hover:border-indigo-200 transition-all group">
                <div className="flex items-start justify-between gap-4 mb-4">
                  <div className="flex items-center gap-4">
                    <div className={`h-14 w-14 rounded-2xl flex items-center justify-center text-2xl shadow-sm ${
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <Pill tone={daysLeft < 3 ? 'danger' : 'info'} className="text-[10px] py-0.5">
                          {daysLeft < 0 ? 'Encerrado' : daysLeft === 0 ? 'É HOJE!' : `${daysLeft} dias restantes`}
                        </Pill>
                      </div>
                      <div className="flex items-center gap-2 text-sm text-slate-500 mt-1">
                        <Calendar className="h-3.5 w-3.5" />
                        {format(new Date(mission.exam_date + 'T12:00:00'), "dd 'de' MMMM", { locale: ptBR })}
                      </div>
                    </div>
                  </div>
                  <button 
