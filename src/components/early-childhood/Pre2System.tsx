        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-black text-slate-900 tracking-tight">
              NeuroBrilha <span className="text-indigo-600">Pré 2</span>
            </h1>
        ...
        <AnimatePresence mode="wait">
          {currentMode === "map" ? (
            <motion.div
              ...
              exit={{ opacity: 0, x: 50 }}
            >
              <EarlyChildhoodWorldMap grade="pre2" onTrailSelect={handleStartActivity} />
...
const GoalItem = ({ label, done }: { label: string; done: boolean }) => (
  <div className="flex items-center gap-3">
    <div
      className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${done ? "bg-emerald-500 border-emerald-500 text-white" : "border-slate-200"}`}
    >
      {done && "✓"}
    </div>
    <span
      className={`text-sm font-bold ${done ? "text-slate-400 line-through" : "text-slate-700"}`}
    >
      {label}
    </span>
  </div>
