        ...
        <AnimatePresence mode="wait">
          ...
          {(activeSection === "math" || activeSection === "literacy") && (
            <motion.div
              ...
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="bg-white p-12 rounded-[3.5rem] shadow-xl"
            >
              <div className="flex justify-between items-center mb-12">
                <h2 className="text-4xl font-black text-indigo-900">
                  {activeSection === "math"
                    ? "Matemática Inicial"
                    : "Alfabetização Neuroadaptativa"}
                </h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
                {(activeSection === "math" ? MATH_SYSTEM.worlds : LITERACY_SYSTEM.modules).map(
                  (m: any) => (
                    <button
                      key={m.id}
                      onClick={() => {
...
const DashboardCard = ({ title, subtitle, color, icon, description, onClick }: any) => (
  ...
  >
    <div
      className={`absolute top-0 right-0 w-32 h-32 ${color} opacity-10 rounded-bl-[5rem] -mr-8 -mt-8 group-hover:scale-150 transition-transform duration-500`}
    />

    <div
      className={`${color} w-20 h-20 rounded-3xl flex items-center justify-center text-4xl shadow-lg mb-6 text-white`}
    >
      {icon}
    </div>
