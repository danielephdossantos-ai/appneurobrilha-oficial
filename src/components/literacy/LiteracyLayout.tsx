...
export const LiteracyLayout: React.FC<LiteracyLayoutProps> = ({
  ...
  const adaptation = isDyslexic
    ? LITERACY_SYSTEM.adaptations.dyslexia
    : isTEA
      ? LITERACY_SYSTEM.adaptations.tea
      : isTDAH
        ? LITERACY_SYSTEM.adaptations.tdah
        : null;

  return (
    <div
      className={`min-h-screen bg-slate-50 p-4 md:p-8 ${isDyslexic ? "font-dyslexic" : ""}`}
      style={{
