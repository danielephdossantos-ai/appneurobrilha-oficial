...
export function PrintableA4Page({ moduleTitle, page, children }: { moduleTitle: string; page: PrintablePage; children: ReactNode }) {
  return <section className="printable-a4-page" data-audience={page.audience} data-page-id={page.id}>
    <header className="printable-page-header"><div><span className="printable-kicker">NeuroBrilha Kids · Caderno de Alfabetização</span><h2>{page.title}</h2><p>{moduleTitle} · Folha {page.number}</p></div><span className="printable-audience">{page.audience === "teacher" ? "Somente para o professor" : "Atividade do estudante"}</span></header>
