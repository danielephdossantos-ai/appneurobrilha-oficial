  ...
  const legacyCompatibility = useMemo(() => {
    ...
    return {
      adaptive: {
        ...
        repetitionRate: 1.0,
        positiveReinforcementFrequency: (adj?.positiveReinforcementFrequency ?? 0.5) > 0.7 ? "high" : "standard",
        autonomyLevel: "guided",
        complexityMultiplier: adj?.difficultyScale ?? 1.0,
        maxItemsPerScreen: adj?.maxInformationDensity ?? 5,
        reinforcementIntensity: (adj?.positiveReinforcementFrequency ?? 0.5) > 0.8 ? "high" : "standard",
