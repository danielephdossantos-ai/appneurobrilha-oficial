...
export class ResponsibleIntelligence {
  static analyzePerformance(data: MockResponsibleData): {
    ...
    const focusTrend: "up" | "down" | "stable" =
      avgFocus > 0.7 ? "up" : avgFocus < 0.4 ? "down" : "stable";

    // Generate Recommendations based on Skills
    const strugglingSkills = data.skills.filter(
      (s) => s.status === "revisao-necessaria" || (s.status === "em-progresso" && s.mastery < 50),
    );
    ...
    if (strugglingSkills.length > 0) {
      recommendations.push({
        ...
        description:
          "O aluno está encontrando dificuldades nesta área. Sugerimos atividades de pareamento visual para consolidar o conceito.",
    ...
    if (frustrationCount > 2) {
      recommendations.push({
        ...
        description:
          "Detectamos picos de frustração recorrentes. Recomendamos mudar para o 'Modo Calmante' nas próximas sessões de lógica.",
    ...
    if (incompleteTasks > 2) {
      recommendations.push({
        ...
        description:
          "A carga de atividades hoje parece estar acima do ideal. Que tal reduzir a quantidade de micro-habilidades por bloco?",
