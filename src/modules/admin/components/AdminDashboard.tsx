    ...
    <div className="p-8 space-y-8 bg-slate-50 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <MetricCard
          ...
          value={mockMetrics.active_users_24h.toString()}
          icon={<Users className="text-blue-500" />}
          change="+12%"
        />
        <MetricCard
          title="Taxa de Retenção"
          value={`${mockMetrics.retention_rate}%`}
          icon={<TrendingUp className="text-green-500" />}
          change="+2.4%"
        />
        <MetricCard
          title="Atividades Realizadas"
          value={mockMetrics.total_activities_today.toLocaleString()}
          icon={<CheckCircle className="text-purple-500" />}
          change="+18%"
        />
        <MetricCard
          title="Erros Críticos"
          value={mockMetrics.critical_errors.toString()}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <QuickAction
          ...
          description="Acessar base completa de perfis"
          icon={<Users />}
        />
        <QuickAction
          title="Gestão BNCC"
          description="Configurar habilidades e objetivos"
          icon={<CheckCircle />}
        />
        <QuickAction
          title="Configurações de Planos"
          description="Gerenciar assinaturas e acessos"
          icon={<CreditCard />}
        />
      </div>
    </div>
  ...
  <Card className="bg-white border-slate-200">
    <CardContent className="pt-6">
      <div className="flex justify-between items-start mb-4">
        ...
        <span
          className={`text-xs font-bold px-2 py-1 rounded-full ${trend === "up" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
        >
          {change}
        </span>
      </div>
