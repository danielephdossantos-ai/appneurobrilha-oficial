...
export class AnamnesisEngine {
  static interpret(data: AnamnesisData): NeuroProfile {
    const scores = {
      ...
      sensory:
        (data.sensory.auditory_sensitivity +
          data.sensory.visual_sensitivity +
          data.sensory.tactile_sensitivity) /
        3,
      behavioral: data.behavioral.routine_stability,
    };

    const adaptation_rules = {
      visual_clutter_reduction:
        data.sensory.visual_sensitivity > 70 || data.cognitive.attention < 40,
      audio_reinforcement_needed:
        data.sensory.auditory_sensitivity < 30 || data.pedagogical.reading_ability < 40,
