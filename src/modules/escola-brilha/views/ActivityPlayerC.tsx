...
import {
  Home, Volume2, ChevronRight, Star, Check, X,
  BookOpen, Image, BarChart3, Lightbulb, Target,
  Menu, Bookmark, Trophy, Focus
} from 'lucide-react';
...
export const ActivityPlayerC: React.FC<Props> = ({ lesson }) => {
  ...
  const handleOption = async (letter: string, isCorrect: boolean) => {
    ...
    if (isCorrect) {
      ...
      setShowExplanation(true);
      setTimeout(() => { setSelectedLetter(null); setShowExplanation(false); }, 3000);
    }
  };
  ...
  const replayAudio = () => {
    ...
    else if (currentScreen.id === 'pontos_chave') s.pontos_chave.points.forEach(p => speak(p.title + '. ' + p.text));
  ...
  const accentClasses: Record<string, { bg: string; text: string; light: string; border: string }> = {
    ...
    green:  { bg: 'bg-emerald-600',text: 'text-emerald-600',light: 'bg-emerald-50',border: 'border-emerald-200' },
    ...
    stone:  { bg: 'bg-stone-600',  text: 'text-stone-600',  light: 'bg-stone-50',  border: 'border-stone-200' },
    ...
    yellow: { bg: 'bg-yellow-600', text: 'text-yellow-700', light: 'bg-yellow-50', border: 'border-yellow-200' },
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
        ...
        {sidebarOpen && (
          ...
          >
            <div className={`bg-gradient-to-b ${lesson.color.from} ${lesson.color.to} p-4 pb-6`}>
              <button onClick={() => setSidebarOpen(false)} className="text-white/70 hover:text-white mb-3">
                <X className="w-5 h-5" />
              </button>
              <p className="text-white/60 text-xs font-black uppercase tracking-widest">{lesson.subject}</p>
            ...
            <div className="flex-1 p-3 space-y-1">
              {SCREENS.map((s, i) => (
                <button
                  key={s.id}
                  onClick={() => { setScreenIndex(i); setSidebarOpen(false); }}
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-left transition ${
                    i === screenIndex ? `${ac.light} ${ac.text} font-black` : 'text-slate-500 hover:bg-slate-50'
                  }`}
                >
            ...
            <div className="p-3 border-t border-slate-100 space-y-1">
              {[
                ...
                <button key={label} className="w-full flex items-center gap-3 px-3 py-2 rounded-xl text-slate-400 hover:bg-slate-50 text-sm">
      ...
      <div className="hidden sm:flex flex-col items-center gap-3 w-14 bg-white border-r border-slate-100 py-4 shrink-0">
        <button onClick={() => setSidebarOpen(true)} className="w-9 h-9 rounded-xl bg-slate-100 hover:bg-slate-200 flex items-center justify-center text-slate-500 transition">
          <Menu className="w-4 h-4" />
        </button>
        <button onClick={() => navigate({ to: '/escola-brilha' })} className="w-9 h-9 rounded-xl hover:bg-slate-100 flex items-center justify-center text-slate-400 transition">
        ...
        {SCREENS.map((s, i) => (
          <button
            ...
            className={`w-9 h-9 rounded-xl flex items-center justify-center transition ${
              i === screenIndex ? `${ac.bg} text-white shadow` : i < screenIndex ? 'text-emerald-500 bg-emerald-50' : 'text-slate-300 hover:bg-slate-100'
            }`}
          >
        ...
        <div className="flex-1" />
        <button onClick={() => setSidebarOpen(true)} className="w-9 h-9 rounded-xl hover:bg-slate-100 flex items-center justify-center text-slate-400 transition">
      ...
      <div className="flex-1 flex flex-col min-h-screen overflow-y-auto">
        ...
        <div className="sticky top-0 z-30 bg-white border-b border-slate-100 px-4 py-3 flex items-center gap-3">
          <button onClick={() => setSidebarOpen(true)} className="sm:hidden w-9 h-9 rounded-xl bg-slate-100 flex items-center justify-center text-slate-500">
            <Menu className="w-4 h-4" />
          </button>
          <button onClick={() => navigate({ to: '/escola-brilha' })} className="sm:hidden w-9 h-9 rounded-xl hover:bg-slate-100 flex items-center justify-center text-slate-400">
            <Home className="w-4 h-4" />
          </button>
          ...
          <div className={`flex items-center gap-1.5 ${ac.light} ${ac.border} border px-3 py-1.5 rounded-full`}>
        ...
        <div className="px-4 pt-3 pb-1">
          <div className="flex items-center gap-2 flex-wrap">
            <span className={`text-xs font-black px-3 py-1 rounded-full ${ac.bg} text-white`}>{lesson.area_label}</span>
            <span className="text-xs font-bold text-slate-400">{lesson.grade} • BNCC {lesson.bncc_code}</span>
            <span className="text-xs text-slate-400 hidden sm:block">— {lesson.bncc_description}</span>
          </div>
        </div>
            ...
            >
              ...
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-black text-slate-700 text-sm uppercase tracking-wide">{currentScreen.label}</span>
                  <span className="text-slate-300 text-xs font-semibold">{screenIndex + 1}/{SCREENS.length}</span>
                </div>
              ...
              {currentScreen.id === 'exploracao' && <ExploracaoScreen lesson={lesson} activeTab={activeTab} setActiveTab={setActiveTab} ac={ac} />}
              {currentScreen.id === 'pontos_chave' && <PontosChaveScreen lesson={lesson} pointsVisible={pointsVisible} ac={ac} />}
              {currentScreen.id === 'exemplo_aplicado' && <ExemploScreen lesson={lesson} pointsVisible={pointsVisible} ac={ac} />}
              {currentScreen.id === 'desafio' && (
                <DesafioScreen lesson={lesson} selectedLetter={selectedLetter} showExplanation={showExplanation}
                  onSelect={handleOption} ac={ac} />
              )}
            </motion.div>
          </AnimatePresence>
  ...
  return (
    <div className="space-y-4">
        ...
        <ul className="space-y-1.5">
          {s.objectives.map((obj, i) => (
            <motion.li key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}
              ...
              <span className="w-5 h-5 rounded-full bg-white/25 flex items-center justify-center text-xs font-black shrink-0">{i + 1}</span>
              <span>{obj}</span>
            </motion.li>
          ))}
        </ul>
      ...
      <div className="flex justify-between items-end px-4">
        <img src={pipaImg} alt="Pipa" className="w-20 h-20 object-contain drop-shadow-lg select-none" draggable={false} />
        <div className={`flex-1 mx-3 ${ac.light} ${ac.border} border rounded-xl p-2 text-center`}>
          ...
          <p className="text-slate-500 text-xs mt-0.5">{lesson.subject} • {lesson.grade}</p>
        </div>
...
const ExploracaoScreen: React.FC<{
  lesson: ActivityLessonC; activeTab: TabId; setActiveTab: (t: TabId) => void; ac: any
  ...
  return (
    <div className="space-y-4">
      ...
      <AnimatePresence mode="wait">
        ...
        {activeTab === 'pontos' && (
          <motion.div key="pontos" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            className="space-y-2">
...
/* ─── TELA 3: PONTOS-CHAVE ─── */
const PontosChaveScreen: React.FC<{ lesson: ActivityLessonC; pointsVisible: number[]; ac: any }> = ({ lesson, pointsVisible, ac }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className={`${ac.light} ${ac.border} border rounded-2xl p-3 flex items-center gap-3`}>
        <img src={pipaImg} alt="Pipa" className="w-12 h-12 object-contain select-none" draggable={false} />
        <p className={`text-sm font-semibold ${ac.text}`}>Anote os pontos mais importantes — vão ajudar no desafio!</p>
      </div>
    </div>
  );
};
    ...
    <div className="space-y-4">
      ...
      <div className="bg-slate-800 rounded-2xl p-4 text-white">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-xs font-black text-yellow-400 uppercase tracking-widest">Desafio</span>
        ...
        {s.options.map((opt) => {
          ...
          return (
            <motion.button
              ...
              `}
            >
              <div className={`w-9 h-9 rounded-xl flex items-center justify-center text-white font-black text-sm shrink-0 ${LETTER_COLORS[opt.letter]}`}>
                {opt.letter}
              </div>
              <p className="text-slate-700 font-semibold text-sm flex-1 leading-snug">{opt.text}</p>
              {showResult && (opt.isCorrect ? <Check className="w-5 h-5 text-emerald-500 shrink-0" /> : <X className="w-5 h-5 text-red-400 shrink-0" />)}
              {highlightCorrect && !showResult && <Check className="w-5 h-5 text-emerald-500 shrink-0" />}
            </motion.button>
          );
        })}
      </div>
      ...
      <AnimatePresence>
        {showExplanation && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            ...
            <div>
              <p className="text-xs font-black text-slate-500 mb-1 uppercase tracking-wide">Explicação</p>
