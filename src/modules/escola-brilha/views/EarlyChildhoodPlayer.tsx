...
import {
  ...
  CountTeachStep, CountPracticeStep, SubtractStep,
    ...
    <LessonEnvironment>
      ...
      <div className="fixed top-0 left-0 right-0 z-50 px-3 pt-3 pb-2 bg-white/80 backdrop-blur-md border-b border-white/60">
        <div className="flex items-center gap-2 max-w-xl mx-auto">
          {onBack && (
            <button onClick={onBack} className="w-9 h-9 rounded-full bg-slate-100 flex items-center justify-center hover:bg-slate-200 transition shrink-0">
...
/* ─── INTRO ──────────────────────────────────────────── */
const IntroSlide: React.FC<StepProps & { step: IntroStep }> = ({ step, setIsSpeaking, onNext, cancelRef }) => {
  useEffect(() => {
    let live = true;
    (async () => {
      setIsSpeaking(true);
  ...
  return (
    <div className="bg-white rounded-3xl shadow-xl border-4 border-violet-200 p-8 flex flex-col items-center gap-5 text-center">
      ...
      <div className="flex gap-1 mt-2">
        {[...Array(3)].map((_, i) => (
          <motion.div key={i} animate={{ y: [0, -6, 0] }} transition={{ repeat: Infinity, delay: i * 0.2, duration: 0.8 }}
...
/* ─── VOWEL TEACH ────────────────────────────────────── */
const VowelTeachSlide: React.FC<StepProps & { step: VowelTeachStep }> = ({ step, setIsSpeaking, onNext, cancelRef }) => {
...
/* ─── VOWEL PRACTICE ─────────────────────────────────── */
const VowelPracticeSlide: React.FC<StepProps & { step: VowelPracticeStep }> = ({ step, setIsSpeaking, onNext }) => {
      ...
      <div className="flex gap-4 flex-wrap justify-center">
        {visible && step.options.map((opt, i) => {
          ...
          const borderClass = isChosen
            ? isCorrect ? 'border-emerald-500 ring-4 ring-emerald-300' : 'border-red-400 ring-4 ring-red-200'
            : 'border-transparent';
          return (
            <motion.button
              key={opt.letter}
...
/* ─── COUNT TEACH ────────────────────────────────────── */
const CountTeachSlide: React.FC<StepProps & { step: CountTeachStep }> = ({ step, setIsSpeaking, onNext, cancelRef }) => {
...
/* ─── COUNT PRACTICE ─────────────────────────────────── */
const CountPracticeSlide: React.FC<StepProps & { step: CountPracticeStep }> = ({ step, setIsSpeaking, onNext }) => {
  const [feedback, setFeedback] = useState<'ok' | 'err' | null>(null);
      ...
      <div className="flex gap-4 justify-center flex-wrap">
        {showButtons && step.options.map((n, i) => {
          ...
          const border = isChosen
            ? isCorrect ? 'border-emerald-500 ring-4 ring-emerald-200' : 'border-red-400 ring-4 ring-red-200'
            : 'border-transparent';
          const bg = isChosen
...
const SubtractSlide: React.FC<StepProps & { step: SubtractStep }> = ({ step, setIsSpeaking, onNext, cancelRef }) => {
...
/* ─── CELEBRATION ─────────────────────────────────────── */
const CelebrationScreen: React.FC<{ lesson: EarlyLesson; onBack?: () => void }> = ({ lesson, onBack }) => {
  useEffect(() => {
    speak(`Parabéns! Você completou ${lesson.title}! Você é muito inteligente! Continue aprendendo!`);
  }, []);
