...
const LegacyLessonPlayer: React.FC = () => {
  ...
  const handleInteraction = async (answer: string) => {
    ...
    if (isCorrect) {
      setPerformance(prev => ({ ...prev, hits: prev.hits + 1, percentage: ((prev.hits + 1) / (prev.hits + prev.misses + 1)) * 100 }));
      ...
      setPerformance(prev => ({ ...prev, misses: prev.misses + 1, percentage: (prev.hits / (prev.hits + prev.misses + 1)) * 100 }));
  ...
  return (
    <LessonEnvironment>
      ...
      <div className={`w-full ${isSecondYear ? 'max-w-2xl px-2' : 'max-w-md px-4'} pt-24 pb-56 flex flex-col items-center`}>
        ...
        >
          <div className="w-full flex items-start gap-2">
            <p className="flex-1 text-center text-lg sm:text-xl font-black text-slate-700 leading-snug">{currentStep.speech}</p>
            <button onClick={replaySpeech} className="shrink-0 w-9 h-9 rounded-full bg-violet-100 hover:bg-violet-200 text-violet-600 flex items-center justify-center active:scale-95 transition" aria-label="Ouvir novamente">
              ...
              <AnimatePresence>
                {currentStep.elements.map((el: any) =>
                  showElements.includes(el.id) && (
                    <motion.div key={el.id} initial={isSecondYear ? { opacity: 0 } : { scale: 0, opacity: 0, y: 30 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0, opacity: 0 }} transition={isSecondYear ? { duration: 0.2 } : { type: 'spring', stiffness: 200, damping: 14 }} className={`flex flex-col items-center justify-center transition-all duration-300 ${highlightedElementId === el.id ? 'scale-110' : 'scale-100'}`}>
                      {(/\p{Emoji}/u.test(el.content) || objetoImg(el.content)) ? (
                        ...
                        <div className={`text-5xl sm:text-6xl font-black px-2 transition-all duration-300 ${highlightedElementId === el.id ? 'text-yellow-400 drop-shadow-[0_0_15px_rgba(250,204,21,0.8)]' : 'text-blue-600'}`}>
                          {el.content}
                        </div>
                      )}
                    </motion.div>
            ...
            <div className="w-full flex flex-wrap items-center justify-center gap-4 pt-4">
              <AnimatePresence>
                {visibleOptions.map((opt, i) => {
                  const palette = ['bg-blue-500 border-blue-700', 'bg-emerald-500 border-emerald-700', 'bg-yellow-400 border-yellow-600', 'bg-pink-500 border-pink-700'];
                  const color = palette[i % palette.length];
                  const hasIllust = /\p{Emoji}/u.test(opt) || objetoImg(opt);
                  return (
                    <motion.button key={opt} initial={{ scale: 0, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} transition={{ type: 'spring', stiffness: 220 }} onClick={() => handleInteraction(opt)} className={`${color} text-white w-24 h-24 sm:w-28 sm:h-28 rounded-3xl text-3xl sm:text-4xl font-black shadow-xl border-b-8 hover:scale-105 active:scale-95 transition flex items-center justify-center p-4`}>
                      {hasIllust ? <RenderEmoji e={opt} className="w-full h-full" label={semEmoji(opt)} /> : <span>{opt}</span>}
                    </motion.button>
                  );
                })}
              </AnimatePresence>
      ...
      <AnimatePresence>
        {feedback && (
          <motion.div initial={{ opacity: 0, scale: 0.6 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} className="fixed inset-0 flex items-center justify-center z-[100] pointer-events-none">
            <div className={`px-8 py-5 rounded-3xl text-3xl sm:text-5xl font-black shadow-2xl border-4 ${feedback.kind === 'ok' ? 'bg-emerald-400 text-white border-emerald-200' : feedback.kind === 'done' ? 'bg-yellow-400 text-white border-yellow-200' : 'bg-pink-400 text-white border-pink-200'}`}>
              {feedback.msg}
            </div>
          </motion.div>
        )}
