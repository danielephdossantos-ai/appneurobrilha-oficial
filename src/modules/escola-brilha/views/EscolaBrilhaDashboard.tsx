...
const STATIC_LESSONS: Record<EtapaEscolar, StaticLesson[]> = {
  ...
  fundamental1: [
    ...
    { id: "matematica", type: "legacy", serie: "1º Ano", title: "Matemática Divertida", subtitle: "Números e quantidades", badge: "Matemática", badgeColor: "bg-emerald-100 text-emerald-700", gradient: "from-emerald-500 to-green-600", xp: 120, bncc: "EF01MA06" },
    { id: "ciencias_1ano", type: "legacy", serie: "1º Ano", title: "Os Cinco Sentidos", subtitle: "Ver, ouvir, cheirar, tocar e provar", badge: "Ciências", badgeColor: "bg-cyan-100 text-cyan-700", gradient: "from-cyan-500 to-teal-600", xp: 120, bncc: "EF01CI02" },
    { id: "historia_1ano", type: "legacy", serie: "1º Ano", title: "Vivendo e Lembrando", subtitle: "Memórias e família", badge: "História", badgeColor: "bg-orange-100 text-orange-700", gradient: "from-orange-500 to-amber-600", xp: 120, bncc: "EF01HI04" },
...
export const EscolaBrilhaDashboard: React.FC = () => {
  ...
  const goToBnccCode = (codigo: string) =>
    navigate({ to: "/bncc/$codigo", params: { codigo } });
      ...
      <div className="px-4 max-w-lg mx-auto">
        <AnimatePresence mode="wait">
          ...
          >
                  ...
                  <div className="flex items-end justify-between mb-3 px-1">
                    <div>
                      ...
                      <p className="text-white/40 text-[11px] font-semibold uppercase tracking-widest">
                        {practicalTotal} {practicalTotal === 1 ? "aula" : "aulas"} • {bnccCards.length} habilidades BNCC
                      </p>
                    </div>
                  </div>
            ...
            {!loading && !loadingBncc && allSeries.every((s) =>
...
const SUBJECT_META: Record<string, { gradient: string; Icon: React.FC<{ className?: string }> }> = {
  ...
  "Arte": { gradient: "from-fuchsia-500 to-pink-600", Icon: Sparkles },
  "Educação Física": { gradient: "from-orange-500 to-red-600", Icon: Sprout },
  "Ensino Religioso": { gradient: "from-violet-500 to-purple-700", Icon: Landmark },
  "Língua Inglesa": { gradient: "from-indigo-500 to-sky-600", Icon: Globe },
};
...
const subjectKey = (raw: string) => {
  ...
  if (s.includes("física") || s.includes("fisica")) return "Educação Física";
...
const SubjectFolders: React.FC<{
  ...
  onBnccClick: (codigo: string) => void;
  ...
  return (
    <div className="space-y-2.5">
      {subjects.map((subj, i) => {
        ...
        const total = statics.length + dbs.length + bnccs.length;
            ...
            >
              ...
              <div className="flex-1 p-3.5 min-w-0">
                ...
                <p className="text-slate-500 text-xs font-medium mt-0.5">
                  {practicalTotal} {practicalTotal === 1 ? "aula" : "aulas"} • {bnccs.length} BNCC
                </p>
              </div>
              <div className="flex items-center pr-3">
                <ChevronRight
                        ...
                        <div className="space-y-2.5">
                          {bnccs.map((h, j) => (
                            <LessonCard
                              ...
                              title={h.codigo}
                              subtitle={h.objeto_conhecimento || h.habilidade || h.unidade_tematica || "Habilidade BNCC"}
                              badge={h.componente}
