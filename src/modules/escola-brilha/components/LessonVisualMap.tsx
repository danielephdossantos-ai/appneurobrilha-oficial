...
export const BiomasBrasilMap: React.FC = () => {
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs font-black text-slate-400 uppercase tracking-widest text-center">Mapa dos Biomas</p>
...
export const OceanosMap: React.FC = () => {
  ...
  return (
    <div className="space-y-2">
      <p className="text-xs font-black text-slate-400 uppercase tracking-widest text-center mb-2">Zonas do Oceano</p>
      {zones.map((z, i) => (
          ...
          {revealed.includes(i) && (
            <motion.div initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }}
              ...
              <div className="flex-1 py-2 pr-2">
                ...
                <p className="text-slate-600 text-[11px] font-medium leading-tight mt-0.5">{z.text}</p>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
...
export const CelulaDiagram: React.FC = () => {
  ...
  return (
    <div className="space-y-2">
      <p className="text-xs font-black text-slate-400 uppercase tracking-widest text-center mb-1">Estrutura da Célula Eucarionte</p>
              ...
              <AnimatePresence>
                {active === p.name && (
                  <motion.p initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                    className="text-slate-600 text-[11px] mt-0.5 font-medium overflow-hidden">{p.text}</motion.p>
                )}
              </AnimatePresence>
            </div>
          </motion.button>
...
export const EquacaoSteps: React.FC = () => {
  ...
  const steps = [
    { expr: '2x + 4 = 12',       color: 'bg-blue-50   border-blue-400   text-blue-800',   label: 'Equação original' },
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs font-black text-slate-400 uppercase tracking-widest text-center">Resolução Passo a Passo</p>
      <div className="space-y-2">
...
export const SistemaNervosoDiagram: React.FC = () => {
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs font-black text-slate-400 uppercase tracking-widest text-center">Sistema Nervoso Central</p>
...
export const RevolucaoTimeline: React.FC = () => {
  ...
  return (
    <div className="space-y-2">
      <p className="text-xs font-black text-slate-400 uppercase tracking-widest text-center mb-2">Linha do Tempo</p>
      {events.map((ev, i) => (
        <AnimatePresence key={ev.year}>
          {revealed.includes(i) && (
            <motion.div initial={{ opacity: 0, x: i % 2 === 0 ? -20 : 20 }} animate={{ opacity: 1, x: 0 }}
              transition={{ type: 'spring', stiffness: 180 }}
...
export const PunnettSquare: React.FC = () => {
  ...
  return (
    <div className="space-y-4">
      <p className="text-xs font-black text-slate-400 uppercase tracking-widest text-center">Quadrado de Punnett</p>
      <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm">
        ...
        <div className="flex justify-center">
          <div className="grid grid-cols-3 gap-1 w-48">
            {/* Header row */}
            <div className="bg-slate-100 rounded-lg p-2 text-center text-sm font-black text-slate-500">×</div>
            <div className="bg-violet-100 rounded-lg p-2 text-center font-black text-violet-700">p</div>
            <div className="bg-violet-100 rounded-lg p-2 text-center font-black text-violet-700">p</div>
            {/* Row 1 */}
            <div className="bg-violet-100 rounded-lg p-2 text-center font-black text-violet-700">P</div>
            <motion.div initial={{ scale: 0 }} animate={{ scale: step >= 1 ? 1 : 0 }}
              className="bg-violet-400 rounded-lg p-2 text-center font-black text-white text-sm">Pp</motion.div>
            <motion.div initial={{ scale: 0 }} animate={{ scale: step >= 2 ? 1 : 0 }}
              className="bg-violet-400 rounded-lg p-2 text-center font-black text-white text-sm">Pp</motion.div>
            {/* Row 2 */}
            <div className="bg-violet-100 rounded-lg p-2 text-center font-black text-violet-700">P</div>
            <motion.div initial={{ scale: 0 }} animate={{ scale: step >= 3 ? 1 : 0 }}
              className="bg-violet-400 rounded-lg p-2 text-center font-black text-white text-sm">Pp</motion.div>
            <motion.div initial={{ scale: 0 }} animate={{ scale: step >= 4 ? 1 : 0 }}
              className="bg-violet-400 rounded-lg p-2 text-center font-black text-white text-sm">Pp</motion.div>
          </div>
        </div>
        {step < 4 ? (
          <button onClick={() => setStep(p => p + 1)} className="mt-3 w-full bg-pink-500 text-white font-black py-2 rounded-xl active:scale-95 transition">
            Preencher →
          </button>
        ) : (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-3 space-y-1">
            <p className="text-xs font-black text-slate-500 text-center mb-2">Resultado — 1ª geração (Pp × Pp) seria:</p>
            {results.map(r => (
              <div key={r.genotype} className={`${r.color} rounded-lg px-3 py-1.5 flex items-center justify-between`}>
                <span className="font-black text-white text-sm">{r.genotype} → {r.phenotype}</span>
...
export const IluminismoVisual: React.FC = () => {
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs font-black text-slate-400 uppercase tracking-widest text-center">Filósofos Iluministas</p>
      <div className="grid grid-cols-2 gap-2">
        {thinkers.map((t, i) => (
