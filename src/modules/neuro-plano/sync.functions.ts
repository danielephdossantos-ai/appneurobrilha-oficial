...
async function authenticatedSyncData(data: Record<string, unknown>) {
...
export async function syncPlansToRoutine({ data }: { data: any }) {
  return syncPlansToRoutineServer({
    data: await authenticatedSyncData(data),
  } as any);
}
