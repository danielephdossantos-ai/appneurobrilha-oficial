...
export class CognitiveEngine {
  ...
  calculateNewScores(
    ...
    outcomes.forEach((outcome) => {
      ...
      // Universal traits
      updated.attention = Math.min(
        100,
        Math.max(0, updated.attention + (outcome.focusDuration > 60 ? 0.2 : -0.1)),
      );
  ...
  processProfileUpdate(
    ...
    newOutcomes.forEach((outcome) => {
      ...
      if (!outcome.success) {
        ...
        const errorCount =
          frequentErrors.find((e) => e.skill_code === outcome.skillCode)?.error_count || 0;
        if (errorCount > 3) {
