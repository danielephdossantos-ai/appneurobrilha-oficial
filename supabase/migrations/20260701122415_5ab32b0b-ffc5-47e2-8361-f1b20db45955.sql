...
BEGIN
  ...
  -- NOVO: Explicação dividida em pequenas etapas (>=2)
  IF jsonb_typeof(c->'explicacao_etapas') <> 'array' OR jsonb_array_length(c->'explicacao_etapas') < 2
    THEN faltando := faltando || 'explicacao_etapas (>=2)'; END IF;
  ...
  -- NOVO: Mensagem final de incentivo obrigatória
  IF COALESCE(c->>'mensagem_final','') = '' THEN
    faltando := faltando || 'mensagem_final';
  END IF;
