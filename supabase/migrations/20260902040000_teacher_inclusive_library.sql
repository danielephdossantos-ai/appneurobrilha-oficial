...
-- Rota interna da aula curricular original. O ALTER também corrige instalações
-- que já tenham executado uma versão anterior desta migração.
ALTER TABLE public.teacher_inclusive_lessons
  ADD COLUMN IF NOT EXISTS source_route text;
ALTER TABLE public.teacher_inclusive_lessons
  DROP CONSTRAINT IF EXISTS teacher_inclusive_lessons_source_route_internal;
ALTER TABLE public.teacher_inclusive_lessons
  ADD CONSTRAINT teacher_inclusive_lessons_source_route_internal
  CHECK (source_route IS NULL OR (source_route LIKE '/%' AND source_route NOT LIKE '//%' AND source_route NOT LIKE '%://%'));
