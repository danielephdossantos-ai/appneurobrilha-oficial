...
BEGIN
  ...
  FOR _item IN SELECT * FROM jsonb_array_elements(COALESCE(_payload->'adaptacoes','[]'::jsonb)) LOOP
    ...
    _perfil_norm := CASE
      WHEN _perfil_raw IN ('tea', 'autismo', 'transtorno do espectro autista') THEN 'tea'
      WHEN _perfil_raw IN ('tdah', 't.d.a.h') THEN 'tdah'
      WHEN _perfil_raw IN ('dislexia', 'disléxia') THEN 'dislexia'
      WHEN _perfil_raw IN ('deficiencia_intelectual', 'deficiência intelectual', 'deficiencia intelectual') THEN 'deficiencia_intelectual'
      WHEN _perfil_raw IN ('altas_habilidades', 'altas habilidades', 'superdotacao', 'superdotação') THEN 'altas_habilidades'
      ELSE 'tdah'
    END;
