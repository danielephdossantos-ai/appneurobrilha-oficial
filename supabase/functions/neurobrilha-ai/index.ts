...
serve(async (req) => {
  ...
  try {
    ...
    if (!child) {
      ...
      return new Response(
        JSON.stringify({ error: "Os dados da criança são obrigatórios para a personalização." }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
    ...
    if (mode === "terapeuta") {
      ...
      if (bncc_code) {
        ...
        if (techs && techs.length > 0) {
          tecnicasContext =
            ...
            techs
              .map(
                (t: any) => `- ${t.tecnicas_pedagogicas.nome}: ${t.tecnicas_pedagogicas.descricao}`,
              )
              .join("\n");
        }
      }
      ...
      const tierLabel = isEarlyYears
        ? "Educação Infantil (Pré + 1º)"
        : isMid
          ? "Anos Iniciais (2º ao 5º)"
          : isTeen
      ...
      const isAlfabetizacao =
        (isEarlyYears || isMid) && (subject === "portugues" || subject === "linguagem");
      const isMathInicial =
        (isEarlyYears || isMid) && (subject === "matematica" || subject === "numeros");
