...
function generateBNCCMatrix() {
  ...
  components.forEach((comp) => {
    ...
    years.forEach((year, idx) => {
      ...
      report[comp][year] = {
        ...
        missing,
        percentage:
          skillsInYear.length > 0
            ? ((implemented.length + partial.length * 0.5) / skillsInYear.length) * 100
            : 0,
      };
    });
  });
  ...
  components.forEach((comp) => {
    ...
    years.forEach((year) => {
      ...
      if (data.total > 0 || year === "1º Ano" || year === "Educação Infantil") {
        const status =
          data.percentage >= 80 ? "✅ ALTA" : data.percentage >= 40 ? "⚠️ MÉDIA" : "❌ BAIXA";
        console.log(
          `${year.padEnd(18)} | Cobertura: ${data.percentage.toFixed(1)}% | Status: ${status}`,
        );
        if (data.implemented.length > 0)
          console.log(`   - Totalmente: ${data.implemented.join(", ")}`);
  ...
  const totalImplemented = BNCC_SKILLS.filter((s) =>
    TEMPLATES.some((t) => t.bnccCodes.includes(s.code)),
  ).length;
