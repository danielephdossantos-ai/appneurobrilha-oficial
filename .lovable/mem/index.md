...
**CONTRATO Matemática 3º ano+ — atividade segue a explicação, não o contrário**: o formato visual é escolhido pelo CONTEÚDO da aula, nunca por default. Sequências/ordenar/comparar/arredondar → linha ou lista dos termos, NUNCA casas de valor. Numeração/valor posicional/soma/subtração armada → casas de valor. Tabuada/mult/div → tabuada interativa + referência. Geometria → figura com vértices A,B,C,D e ângulos. Estatística → gráfico real. Medidas → régua/relógio/balança. Frações → figura repartida. Antes de escrever/editar `exemploReal` ou `momento05_modelagem`, checar se o `casasValor` faz sentido pro tópico — se não faz, NÃO incluir. Ver [Matemática formato por conteúdo](mem://preferences/matematica-formato-por-conteudo).
